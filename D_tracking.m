...
% net_online.move('gpu');
% net_online.eval({'input1',gpuArray(input{1}),'input2',gpuArray(input{2}),'input3',gpuArray(input{3}),'input4',gpuArray(featPCA1st)});
...
for i=2:nFrame       
    ...
    net1.eval({'input',gpuArray(patch1)}); % extract features
...
%      feat_update1{cur} = featPCA{1};   
%      feat_update2{cur} = featPCA{2};
%      feat_update3{cur} = featPCA{3};
